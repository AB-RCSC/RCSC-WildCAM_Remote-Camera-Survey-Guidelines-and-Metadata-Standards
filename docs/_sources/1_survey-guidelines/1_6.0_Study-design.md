...
Refer to [Appendix A - Table A2](#TOC_surv_guidelines_table_a2) for additional recommendations on [camera spacing](#camera_spacing) for the different [modelling approaches](#mods_modelling_approach).
...
The [**total number of camera days**](#total_number_of_camera_days) is the number of days that all cameras were active during the [survey](#survey). [Appendix A - Table A2](#TOC_surv_guidelines_table_a2) provides recommendations on the minimum number of [total number of camera days](#total_number_of_camera_days).
